...
module.exports = class TaxEngine2025 {
  ...
  calculateTax(grossIncome, ageGroup, deductions, medicalMembers, medicalDependants) {
    ...
    for (const b of this.taxBrackets) {
      if (taxable > b.min) {
        if (taxable <= b.max || b.max === Infinity) {
          tax = b.base + (taxable - b.min) * b.rate;
          break;
        }
      }
    }
    ...
    const medCredits = (medicalMembers * this.medical.main * 12) +
                       (Math.min(medicalDependants, 1) * this.medical.firstDep * 12) +
                       (Math.max(0, medicalDependants - 1) * this.medical.extraDep * 12);
